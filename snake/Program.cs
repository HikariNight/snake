    ...
    public static void Main()
    {
        ...
        while (!Raylib.WindowShouldClose())
        {
            if (startScreen)
            {
                ...
                Raylib.DrawText("https://github.com/HikariNight", 10, 550, 20, new Color(20, 20, 20, 100));
                Raylib.DrawText("Credits to: https://github.com/koterba", 10, 570, 20, new Color(20, 20, 20, 100));
                Raylib.EndDrawing();
            ...
            else if ((keyPressed == 87 || keyPressed == 265) && snake.GetDirection() != Direction.Down)
            { // w key or up arrow
                snake.SetDirection(Direction.Up);
            }
            else if (keyPressed == 83 || keyPressed == 264 && snake.GetDirection() != Direction.Up)
            { // s key or down arrow
                snake.SetDirection(Direction.Down);
            }
            else if (keyPressed == 65 || keyPressed == 263 && snake.GetDirection() != Direction.Right)
            { // a key or left arrow
                snake.SetDirection(Direction.Left);
            }
            else if (keyPressed == 68 || keyPressed == 262 && snake.GetDirection() != Direction.Left)
            ...
            Raylib.ClearBackground(new Color(218, 74, 74, 255));

            // play area is drawn as a rectangle on top of the background
            Raylib.DrawRectangle(20, 20, 560, 560, new Color(158, 58, 58, 255));
            ...
            foreach (Position segment in segments)
            {
                if (segmentIndex == 0)
                {
                    Raylib.DrawRectangle(segment.X, segment.Y, 20, 20, new Color(31, 31, 31, 255));
                    Raylib.DrawCircle(segment.X + 10, segment.Y + 10, 3, new Color(235, 219, 219, 100));
